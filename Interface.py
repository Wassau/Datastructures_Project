...
sheet2 = workbook['Registros']

Usuarios = ArrayList.ArrayLinerList(10)


i = 0
for id_user, USERNAME, PASSWORD, NOMBRE, FECHA_NAC, REGION, PRIORITY in sheet.iter_rows():
    
    arr = ArrayList.ArrayLinerList(10)
    for id, SatOx,FrCard,Hora,Fecha in sheet2.iter_rows():

        if(i+1 == (id.value)):
            registro = Registro((id.value), SatOx.value, FrCard.value, str(Hora.value), str(Fecha.value))
...
colaUs = Priorityqueue.Priorityqueue()

for i in range(0,Usuarios.size):
    colaUs.insertitem(Usuarios.get(i))

colaDoc = ArrayQueue(200)

for i in range(0,Medicos.size):
    if Medicos.get(i).disponible == True:
        colaDoc.put(Medicos.get(i))

sheetCitas = workbook["Citas"]

while colaDoc.isEmpty() == False:
    sheetCitas.cell(len(sheetCitas['A'])+1,1).value = colaUs.Array[0].id
    colaUs.Removemax()
    sheetCitas.cell(len(sheetCitas['B']),2).value = colaDoc.remove().id
    sheetCitas.cell(len(sheetCitas['C']),3).value = now.date()
    sheetCitas.cell(len(sheetCitas['D']),4).value = now.time()
